...
class Solver:
    ...
    def solve(self, rhs: TreeVector,
              out: TreeVector | None = None, *,
              graph: bool = True) -> TreeVector:
        ...
        ``graph=False`` enqueues the prepared solve launches directly on
        the current stream instead of replaying the captured solve
        graph.  This is for callers that capture a larger CUDA graph of
        their own (a replayed graph cannot be recorded into another
        capture); warm the binding with one ordinary ``solve()`` on the
        same buffers first so kernel compilation happens outside the
        capture.
        """
