...
PHASE_GROUPS = {
    "overhead": ["phase_refresh_ms"],
    "tail factor": ["phase_tail_factor_ms"],
    "tail solves": ["phase_tail_forward_ms", "phase_tail_backward_ms"],
    "root update": ["phase_root_diagonal_update_ms",
                    "phase_root_diagonal_reduce_ms",
                    "phase_root_rhs_update_ms"],
    "root": ["phase_root_factor_ms", "phase_root_solve_ms",
             "phase_root_store_ms"],
    "recovery": ["phase_tail_rhs_correction_ms"],
}
PHASE_COLORS = ["#c3c2b7", "#2a78d6", "#1baf7a", "#eb6834", "#eda100",
                "#e87ba4"]

SWEEP_PARAMS = {
    "tails": ("num_tails", "number of tails B"),
    "horizon": ("horizon", "horizon N"),
    "block_size": ("block_size", "stage block size n"),
    "root": ("root_dim", "root dimension m"),
}
...
def fig_phase_breakdown(df, out_dir):
    d = df[(df["method"] == "tree_socu") & (df["status"] == "ok")
           & df["sweep"].isin(["tails", "total_scaling", "default_point"])]
    if not len(d):
...
def fig_accuracy_vs_size(df, out_dir):
    d = df[(df["status"] == "ok")
           & df["sweep"].isin(["tails", "horizon", "total_scaling"])]
    if not len(d):
...
def fig_heatmap(agg, out_dir):
    ...
    if not len(d):
        ...
        return
    tab = d.pivot_table(index="horizon", columns="num_tails",
    ...
    ax.set_xlabel("tails B"); ax.set_ylabel("horizon N")
...
def fig_cold_warm(df, out_dir):
    ...
    d = d[(d["num_tails"] == 64) & (d["horizon"] == 128)] if len(
        d[(d["num_tails"] == 64) & (d["horizon"] == 128)]) else d
    if not len(d):
...
def fig_target_points(df, out_dir):
    ...
    panels = [("target_points", "FP64", ["tree_socu", "cudss"]),
              ("target_points_fp32", "FP32", ["tree_socu", "cudss"])]
