...
The serialized parameter names (``num_tails``, ``horizon``,
``block_size``, ``root_dim``, ``num_rhs``) are part of the stable
...
_PARAM_ALIASES = {
    "B": "num_tails", "N": "horizon", "n": "block_size",
    "m": "root_dim", "nrhs": "num_rhs",
}

_FLOAT_KEYS = {"rho_tail", "rho_sep", "condition_target"}
_INT_KEYS = {"num_tails", "horizon", "block_size", "root_dim",
...
def case_id(sweep: str, point: dict, seed: int, method_name: str) -> str:
    ...
    tag = (f"B{spec.num_tails}_N{spec.horizon}_n{spec.block_size}_"
           f"m{spec.root_dim}_q{spec.num_rhs}_"
