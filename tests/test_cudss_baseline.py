...
SMALL = ProblemSpec(num_branches=3, horizon=5, block_size=8,
                    separator_dim=6, seed=7)
...
def test_cudss_refactorize_reuses_analysis():
    ...
    assert validation.compute_metrics(s, v.branch,
                                      v.separator)["forward_error"] <= 1e-9
