...
:mod:`src._socu_fused` against SOCU's existing full-solve and factor
...
def _factored_system(B, N, n, q, wp_dtype, seed=3):
    spec = ProblemSpec(num_tails=B, horizon=N, block_size=n,
                       root_dim=4, seed=seed,
...
def test_forward_operator_identity(B, N, n):
    """F^T F = K^{-1}: the identity the factor-space root update relies
    ...
    # Gram matrices per tail: W^T W vs rhs^T K^-1 rhs
