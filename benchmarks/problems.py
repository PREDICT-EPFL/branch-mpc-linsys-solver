...
scenario-tree systems; the ``factor`` mode builds every branch matrix from
a known lower block-bidiagonal factor (``K_i = L_i L_i^T``) and the root
matrix from a prescribed target Schur complement ``S_0 = Z Z^T + delta I``,
so the assembled system is SPD by construction and two exact invariants
are available for validation:

- the separator Schur complement of the generated system equals ``S_0``;
...
_COUPLING_PATTERNS = ("dense", "root_only", "terminal_only", "banded_separator")
...
class ProblemSpec:
    ...
    block_size : int
        Branch-stage block dimension ``n_b`` (>= 1).
    separator_dim : int
    ...
    coupling_pattern : str
        ...
        ``"banded_separator"``.
    band_width : int
        Width of the separator range each stage couples to when
        ``coupling_pattern == "banded_separator"``.
    rho_tail : float
        Scale of the sub-diagonal factor blocks (branch coupling strength).
    rho_sep : float
        Scale of the branch-to-separator coupling blocks.
    ...
    generator_mode : str
        ``"factor"`` (default, SPD by construction with known Schur
    ...
    def shape(self) -> TreeShape:
        ...
        return TreeShape(num_branches=self.num_branches,
                         num_stages=self.horizon,
                         branch_block_dim=self.block_size,
                         separator_dim=self.separator_dim,
...
class GeneratedProblem:
    ...
    and ``schur_target`` exist only because the generator constructs the
    ...
    schur_target: np.ndarray    # (n_y, n_y) exact separator Schur complement
...
def _coupling_mask(spec: ProblemSpec) -> np.ndarray:
    """Stage-by-separator mask (T, n_y) implementing the coupling
    pattern."""
    T, n_y, w = spec.horizon, spec.separator_dim, spec.band_width
    mask = np.zeros((T, n_y))
    if spec.coupling_pattern == "dense":
        ...
        mask[T - 1, :] = 1.0
    elif spec.coupling_pattern == "banded_separator":
        w = min(w, n_y)
        span = max(T - 1, 1)
        for t in range(T):
            start = int(round(t * (n_y - w) / span)) if n_y > w else 0
            mask[t, start:start + w] = 1.0
    return mask


def _tail_factors(spec: ProblemSpec):
    """Draw the known branch factors (L diagonal blocks, F sub-diagonal
    blocks) and the per-block conditioning scales."""
    B, T, n_b = spec.num_branches, spec.horizon, spec.block_size
...
def _make_root(spec: ProblemSpec, schur_update):
    """Target Schur complement ``S_0`` and root matrix ``R``."""
    n_y = spec.separator_dim
    rng = _rng(spec, _SEED_ROOT)
    Z = rng.standard_normal((n_y, n_y)) / np.sqrt(n_y)
    delta = 0.1
    schur_target = Z @ Z.T + delta * np.eye(n_y)
    R = schur_target + schur_update
    R = 0.5 * (R + R.T)
    return schur_target, R
...
def generate_problem(spec: ProblemSpec, estimate_condition=True,
    ...
    ground truth (``exact_solution``, ``schur_target``) stays in FP64 for
    ...
    C_T = rng_c.standard_normal((B, T, n_b, n_y))
    C_T *= spec.rho_sep / np.sqrt(B * T * n_b)
    C_T *= _coupling_mask(spec)[None, :, None, :]

    # ---- branch matrices and the exact Schur update ------------------------
    if spec.generator_mode == "factor":
        ...
        if spec.condition_target is not None:
            ...
            # the Schur update stays O(rho_sep^2) despite ill conditioning.
            C_T *= scales[:, :, None, None]
        D, E = _tail_blocks_from_factors(L, F)
        G = _forward_substitute_known(L, F, C_T)
        schur_update = np.einsum("btij,btil->jl", G, G)

        def branch_solve(rhs):
    ...
    schur_update = 0.5 * (schur_update + schur_update.T)
    schur_target, R = _make_root(spec, schur_update)
    gamma = float(np.linalg.norm(schur_update) / np.linalg.norm(R))

    # ---- true solution and structural right-hand side ----------------------
    rng_x = _rng(spec, _SEED_TRUTH)
    w_true = rng_x.standard_normal((B, T, n_b, spec.num_rhs))
    y_true = rng_x.standard_normal((n_y, spec.num_rhs))
